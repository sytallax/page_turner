...
class Camera(multiprocessing.Process):
    ...
    def take_picture(self):
        ...
        frame = cv2.adaptiveThreshold(
            frame, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 5
        )
        ...
        # resize
        frame = cv2.resize(
            frame, (SCORE_WIDTH, SCORE_HEIGHT), interpolation=cv2.INTER_AREA
        )
        org_frame = cv2.resize(
            org_frame, (SCORE_WIDTH, SCORE_HEIGHT), interpolation=cv2.INTER_AREA
        )
        ...
        score = 1 - np.array(padded_score, dtype=np.float32) / 255.0

        scaled_score = cv2.resize(
            score, (SCALE_WIDTH, SCALE_WIDTH), interpolation=cv2.INTER_AREA
        )

        org_score = np.array(org_frame, dtype=np.float32) / 255.0
...
if __name__ == "__main__":
    ...
    while True:
        if p_input.poll():
            ...
            for s in system_ys:
                cv2.line(
                    org_img, (0, s[0]), (org_img.shape[1], s[0]), color=(0, 255, 0)
                )
                cv2.line(
                    org_img, (0, s[1]), (org_img.shape[1], s[1]), color=(0, 255, 0)
                )
